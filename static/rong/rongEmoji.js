...
(function (a) {
  var b = (function () {
    ...
    c.createSpan = function (e) {
      ...
      if (this.supportEmoji) { f.textContent = e.tag } else {
    ...
    c.calculateUTF = function (g) { var f = this; if (g.charCodeAt(0) > 61440) { var e = f.emojiFactory[escape(g).replace('%u', 'u1')]; if (e) { return e.tag } } return g }
    c.init = function (g) {
      var f = this
      f.url = (a.RongIMClient && a.RongIMClient._memoryStore && a.RongIMClient._memoryStore.depend && a.RongIMClient._memoryStore.depend.emojiImage) || '//cdn.ronghub.com/css-sprite_bg-2.1.10.png'
      if (g) {
        f.emojiFactory = g.dataSource
        f.url = g.url
      }
      if (!g && navigator.userAgent.match(/Mac\s+OS/i)) { f.supportEmoji = true }
      f.initCSS()
      var d = new RegExp('%', 'g'),
        i = '',
        h = ''
      ...
      i = i.replace(d, function (j) { return '\\' })
      f.regExpTag = new RegExp('(' + i + ')', 'g')
      h = h.substring(0, h.length - 1)
      h = h.replace(d, function (j) { return '\\' })
      f.regExpName = new RegExp('(' + h + ')', 'g')
    }
    c.emojiToSymbol = function (e) {
      var d = this
      e = e.replace(/[\uf000-\uf700]/g, function (f) { return d.calculateUTF(f) || f })
      if (d.supportEmoji) { return e }
      return e.replace(d.regExpTag, function (g) { for (var f in d.emojiFactory) { if (d.emojiFactory[f].tag == g) { return '[' + d.emojiFactory[f].zh + ']' } } })
    }
  ...
  a.RongIMEmoji = b
  if (typeof require === 'function' && typeof module === 'object' && module && module.id && typeof exports === 'object' && exports) { module.exports = b } else { if (typeof define === 'function' && define.amd) { define('RongIMEmoji', [], function () { return b }) } }
})(RongIMLib || (RongIMLib = {}))
