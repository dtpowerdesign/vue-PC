<template>
  <div class="perregister">
   <div class="top">
   <div class="top-left"><img src="../../../static/logo-buleBGC.png" alt=""><span onclick="window.location.href='http://39.106.34.156:8080/zs/home/'">{{msg}}</span><span>|</span><span @click="$router.push('/perregister')" style="color:yellow">个人注册</span><span @click="$router.push('/comregister')">企业注册</span><span @click="$router.push('/login')">登录</span></div>
    ...
    submitForm (formName) {
      if (!this.checked) {
        ...
        this.$refs.perregister.validate((valid) => {
          if (valid) {
            this.$http.post(this.$domain.domain1 + 'electric-design/PRegister', {
