<template>
  ...
  <div style="display:flex; align-items:center; justify-content: space-between; background:#F9F9F9;height:4rem">
        <span style="font-size:1.5rem;color:#4d83e7">|合同执行中</span>
          <div>
            <el-button size="small" style='margin-right:20px;' type="warning" icon="document" @click="$router.push('/changeTable/phtzixing')" >表头编辑</el-button>
  ...
  methods: {
    initData () {
      ...
      this.$http.post(this.$domain.domain1 + 'electric-design/getDataFormatOfTables', {'tables': ['projects', 'contracts']}).then((res) => {
        ...
        // this.$http.post(this.$domain.domain1 + 'electric-design/getShowKeyAndExplain', {'belongToUser': this.$cookie.get('user'), 'table': 'projects', 'otherName': 'phtzixing'})
        this.$http.post(this.$domain.domain1 + 'electric-design/getShowKeyAndExplainOfTables', {'belongToUser': this.$cookie.get('user'), 'tables': ['projects', 'contracts'], 'otherName': 'phtzixing'})
      .then((res) => {
        ...
        this.$http.post(this.$domain.domain1 + 'electric-design/getHtzxzProject', formData)
        .then((res) => {
          this.tableData = []
          if (res.data !== 0) {
    ...
    handleDownload () {
      ...
      if (this.multipleSelection.length !== 0) {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, '项目合同执行中信息excel')
    ...
    endContractB () {
      ...
      .then((res) => {
        if (res.data.result) {
          ...
          this.$router.push('/per/PM-sumary/5')
        } else {
          this.$message({
            type: 'warning',
            message: `失败原因:${res.data.reason}`
