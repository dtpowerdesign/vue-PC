<template>
  ...
  <div v-if="show" v-loading="loadingDetail">
    ...
    <el-button type="primary" @click="confirm()" v-if="sourceAccount===$cookie.get('user')">进行投标</el-button>
  </div>
  ...
  <div style="display:flex; align-items:center; justify-content: space-between; background:#F9F9F9;height:4rem">
        <span style="font-size:1.5rem;color:#4d83e7">|发布中</span>
          <div>
            <el-button size="small" style='margin-right:20px;' type="warning" icon="document" @click="$router.push('/changeTable/prelease')" >表头编辑</el-button>
  ...
  methods: {
    initData () {
      ...
      this.$http.post(this.$domain.domain1 + 'electric-design/getDataFormatOfProject').then((res) => {
        ...
        this.$http.post(this.$domain.domain1 + 'electric-design/getShowKeyAndExplain', {'belongToUser': this.$cookie.get('user'), 'table': 'projects', 'otherName': 'prelease'})
      .then((res) => {
    ...
    handleDownload () {
      ...
      if (this.multipleSelection.length !== 0) {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, '项目发布中信息excel')
